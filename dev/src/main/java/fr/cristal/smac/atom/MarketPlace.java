...
public class MarketPlace {
    ...
    public void printState() { // on ne passe que par market maintenant
        for (OrderBook ob : orderBooks.values()) {
            ob.printState();
        }
    }

    public void send(Agent a, Order lo) { // on ne passe que par market maintenant
        lo.sender = a;
    ...
    // This function is for returning an order's rank in
    // orderbook as it enters.
    public int getOrderRank(LimitOrder lo) {
        if (!orderBooks.containsKey(lo.obName)) {
            throw new RuntimeException(lo.obName + " is not present in the market");
        }
        ...
        if (lo.direction == 'A') {
            for (OrderBook ob : orderBooks.values()) {
                return ob.ask.headSet(lo).size() + 1;
            }
        }
        if (lo.direction == 'B') {
            for (OrderBook ob : orderBooks.values()) {
                return ob.bid.headSet(lo).size() + 1;
            }
        }
        return 0;
    }
