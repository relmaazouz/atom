...
public class OrderBook {
    ...
    void printState() {
        ...
        System.err.println(obName + ": nb fixed prices     : " + numberOfPricesFixed
                + (lastPrices.isEmpty() ? "" : "\t (last : " + lastFixedPrice + ")"));
        System.err.println(obName + ": leaving ask size    : " + ask.size()
                + (ask.isEmpty() ? "" : "\t (best : " + ask.first() + ")"));
        System.err.println(obName + ": leaving bid size    : " + bid.size()
                + (bid.isEmpty() ? "" : "\t (best : " + bid.first() + ")"));
    ...
    // 10 means that we cumulate 9 rentabilities, thus computed on the 10 last
    // prices.
    ...
    protected synchronized void fixingEachContinuous() {
            ...
            if (!MarketPlace.acceptNegativeValues) // booleen declare dans MarketPlace
            {
                ...
                if (bbid.sender.getInvest(obName) < 0 || bask.sender.getInvest(obName) < 0)
                    throw new RuntimeException(
                            "invest négatif : " + bbid.sender.getInvest(obName) + " " + bask.sender.getInvest(obName));
            }
        ...
        if (MarketPlace.logType == MarketPlace.SHORT)
            if (localLastPrice != -1 && cumulQuty != 0) {
                PriceRecord pc = new PriceRecord(obName, localLastPrice, cumulQuty, newer.direction,
                        newer.sender.name + ";" + newer.extId, "-");
                setNewPrice(pc, bask, bbid);
    ...
    protected synchronized void fixingEndPreopening() {
        ...
        if (ask.isEmpty() || bid.isEmpty()
                || ask.first().price > bid.first().price) // System.err.println("No matching price at the end of Fix
                                                          // period");
